...
function updateStoredState (name, updates) {
    const current = getStoredState(name);
...
class Puzzle extends React.Component {
    renderLetter (character, i) {
        ...
        const cyphered = this.props.cypher[charCode],
            cypheredCharCode = getCharCode(cyphered),
            charUpper = character.toUpperCase(),
            isHinted = this.props.hints.has(charUpper),
            disabled = this.props.disabled || isHinted;
    ...
    render () {
        const words = this.props.quote.split(" "),
            wordOffsets = Array(words.length).fill(0);
...
function sattoloCycle (items, filterReverseSet) {
    ...
    const reverseMap = Array(items.length).fill(null);
...
class Game extends React.Component {
    ...
    handleChange (i, cyphered) {
        const letter = document.getElementById(`input-${i}`).value,
            solveAttempt = this.state.solveAttempt.slice();
        ...
        if (!this.checkPuzzle(solveAttempt)) {
            ...
            for (let j = 1; j < this.state.quote.Quote.length; ++j) {
                const idx = (j + i) % this.state.quote.Quote.length,
                    character = this.state.quote.Quote[idx].toUpperCase(),
                    charCode = getCharCode(character);
        ...
        this.storeGameState({solveAttempt});
    }
    ...
    render () {
        ...
        const navButtons = [],
            gameNumber = `Playing game ${this.state.quoteNumber + 1} of ${quotes.length}`;
